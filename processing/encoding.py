...
class BSAEncoder:
    ...
    def plot(self):
        ...
        axs[0].plot(self._last_signal)
        axs[0].set_title('Input series')
        axs[1].eventplot(self._last_spike_times)
        axs[1].set_title('BSA Output spike train')
        plt.show()

    def decode(self, plot=False):
        ...
        if plot:
            ...
            plt.suptitle('Decoded series from BSA spike train.')
            plt.legend()
            plt.show()
        return decoded
